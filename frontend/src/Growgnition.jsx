...
const ONBOARDING_STEPS = [
  ...
  { selector: '[data-tour="avail-toggle"]', placement: "bottom", page: "availability", title: "Cards or capacity heatmap",
    body: "Flip to the Capacity heatmap to see every person's allocation week by week — including planned future weeks. Create Task finds the best-fit person for new work using skills tagged by practice heads.",
    narration: "Flip this toggle to the capacity heatmap to see every person's allocation week by week, including planned future weeks. And Create Task ranks the best-fit people for new work using skills tagged by practice heads." },
  { selector: '[data-tour="nav-projects"]', placement: "right", page: "projects", title: "Delivery Engine",
    body: "Every project's delivery picture — work packages, deadlines, owners and the team — scoped to the departments you can see. The Bench Radar lives here too: it flags who frees up in the coming weeks, with AI task suggestions one click away.",
    narration: "This is the Delivery Engine — every project's delivery picture: work packages, deadlines, owners and the team behind each one, scoped to the departments you can see. The Bench Radar lives here too — it flags who frees up in the coming weeks, and one click asks me to suggest their next assignment." },
...
// Official Google "G" mark (4-colour). Transparent centre, so it sits cleanly
// on any background in light or dark mode.
          ...
          {!loading && status?.configured && (
            status.connected ? (
              ...
              <button onClick={connect} disabled={busy} style={{
                ...
                background: COLORS.surface, color: COLORS.textPrimary,
                fontWeight: 700, fontSize: 13, cursor: busy ? "default" : "pointer", display: "inline-flex", alignItems: "center", gap: 9,
                boxShadow: "0 1px 2px rgba(0,0,0,0.08)",
              }}><GoogleG size={17} /> {busy ? "Redirecting…" : "Connect Google Calendar"}</button>
            )
          )}
        </div>
